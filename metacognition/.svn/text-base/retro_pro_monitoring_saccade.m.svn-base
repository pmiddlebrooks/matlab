function retro_pro_monitoring_saccade(soaArrayScreenFlips, nTarget, targetAmplitude, targetAngles, betAngle, targetDistribution, plotFlag, saveFlag)
...
while runningTask
    ...
    stagePerceptionPreFix     	= 1;
    stagePerceptionFix       	= 0;
    ...
    while stagePerceptionPreFix
        ...
        if inAcceptWindow(eyeX, eyeY, fixationWindowPixel) && toc <= graceObtainFixation
            ...
            iPerceptionFixation      = GetSecs - iTrialOnsetComputerTime;
            stagePerceptionPreFix = 0;
    ...
    if stagePerceptionFix
        ...
        while stagePerceptionFix
            ...
            if ~inAcceptWindow(eyeX, eyeY, fixationWindowPixel) && toc < iPreTargetFixDuration
                iTrialOutcome       = 'perceptionFixationAbort';
    ...
    if retroTrialFlag
        ...
        while stageBetPreFix
            ...
            elseif toc > iBetFixSpotOnset + graceObtainFixation
                % Flag it was target aborted
                iTrialOutcome = 'retroInterstageAbort';
                iAbortOnset         = GetSecs - iTrialOnsetComputerTime;
                abortTrialFlag = 1;
                stageBetPreFix = 0;
                stageFeedback = 1;
...
%                 [~, StimulusOnsetTime, ~, ~, ~] = Screen('Flip', window);
    ...
    elseif proTrialFlag
        ...
        if stageDecisionPreFix
            ...
            while stageDecisionPreFix
                ...
                if inAcceptWindow(eyeX, eyeY, fixationWindowPixel) && toc <= graceObtainFixation
                    ...
                    stageDecisionFix = 1;
                elseif GetSecs > fixationOnsetTime + graceObtainFixation
                    iTrialOutcome       = 'proInterstageAbort';
                    iAbortOnset         = GetSecs - iTrialOnsetComputerTime;
                    abortTrialFlag = 1;
                    Eyelink('Message', 'Fixation Abort');
                    stageDecisionFix = 0;
        ...
        if stageDecisionFix
            ...
            while stageDecisionFix
                ...
                if ~inAcceptWindow(eyeX, eyeY, fixationWindowPixel) && toc < iPreMaskFixDuration
                    ...
                    stageDecisionFix = 0;
                    stageFeedback = 1;
                elseif inAcceptWindow(eyeX, eyeY, fixationWindowPixel) &&  toc >= iPreMaskFixDuration
                    stageDecisionFix = 0;
        ...
        if stageDecisionMaskOn
            ...
            while stageDecisionMaskOn
                ...
                if ~inAcceptWindow(eyeX, eyeY, fixationWindowPixel)
                    ...
                    stageDecisionInFlight = 1;
                elseif inAcceptWindow(eyeX, eyeY, fixationWindowPixel) && toc >= graceResponse
                    trialOutcome = 'decisionResponseAbort';
                    iAbortOnset         = GetSecs - iTrialOnsetComputerTime;
                    abortTrialFlag = 1;
                    Eyelink('Message', 'Response Timed Out');
                    stageDecisionMaskOn = 0;
    ...
    if  addTrialDataFlag
        ...
        retroProFlag            = [retroProFlag; iRetroProFlag];
        
        perceptionFixSpotOnset 	= [perceptionFixSpotOnset; iPerceptionFixSpotOnset];
        perceptionFixation     	= [perceptionFixation; iPerceptionFixation];
        
        preTargetFixDuration   	= [preTargetFixDuration; iPreTargetFixDuration];
        postMaskFixDuration    	= [postMaskFixDuration; iPostMaskFixDuration];
        perceptionTargOnset    	= [perceptionTargOnset; iPerceptionTargOnset];
        soa                     = [soa; iSOA];
        perceptionMaskOnset   	= [perceptionMaskOnset; iPerceptionMaskOnset];
...
SessionData.stimuli.perceptionFixColorRGB         = perceptionFixColor;
SessionData.stimuli.decisionFixColorRGB         = decisionFixColor;
SessionData.stimuli.targetRGB           = targetColor;
SessionData.stimuli.retroCueRGB           = retroCueColor;
SessionData.stimuli.proCueRGB           = proCueColor;
