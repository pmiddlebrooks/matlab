function choice_stop_task(stopMode, effector, SSDArrayScreenFlips, checkerAmp, checkerAngle, targAmp, rightTargAngle, magentaTargSide, plotFlag, saveFlag)
...
if nargin < 3
    ...
    SSDArrayScreenFlips = 6 : 6 : 6 + 6*15;
    checkerAmp = 3;
    checkerAngle = 90;
    targAmp = 10;
    rightTargAngle = 0;
...
% rightTargPropArray     = [.2 0.35 .65 .8];
...
INITIAL_SSD_INDEX       = [6 7 8 8 8 7 6]+1;
graceResponse           = 1.4;
...
clockVector     = clock;
session         = [datestr(now,'mmdd'),  effector];
...
while runningTask
    ...
    if newTrialVariables
       ...
       %   GENERATE THE CHECKERED STIMULUS:
        ...
        for iRow = 1 : nCheckerRow
            ...
            % Fill row with as many minority color checkers as there will be
    ...
    if  addTrialDataFlag
        ...
        fixDuration    = [fixDuration; iFixDuration];
        targOn             = [targOn; iTargOn];
        targDuration          = [targDuration; itargDuration];
        distOn         = [distOn; iDistOn];
        distDuration      = [distDuration; iDistDuration];
        checkerOn     = [checkerOn; iCheckerOn];
        checkerDuration  = [checkerDuration; iCheckerDuration];
        targWindowEntered     = [targWindowEntered; iTargWindowEntered];
        ssd                     = [ssd; iSSD];
        realSSD                 = [realSSD; iRealSSD];
        stopSignalOn               = [stopSignalOn; iStopSignalOn];
        ...
        abortTime              = [abortTime; iAbortTime];
        ...
        targ1CheckerProp = [targ1CheckerProp; iRandomRightP];
    ...
    if ~strcmp(iTrialOutcome, 'choiceStimulusAbort') && ...
            ~strcmp(iTrialOutcome, 'fixationAbort') && ...
            ~strcmp(iTrialOutcome, 'saccadeAbort') && ...
            ~strcmp(iTrialOutcome, 'eyelinkError') && ...
            ~strcmp(iTrialOutcome, 'errorKeyPress')
        iTrial                  = iTrial + 1;
    end
    
    if iTrial == nTrialTotal
        runningTask = 0;
    end
    ...
    if (mod(iTrial, nTrialPerBlock) == 1) && (iTrial ~= 1) && ~strcmp(iTrialOutcome, 'fixationAbort') && ~strcmp(iTrialOutcome, 'saccadeAbort')
        iBlock = iBlock + 1;
        msg = sprintf('%d blocks done! Press the space bar to begin block %d', iBlock -1, iBlock);
...
% Event Properties
% ---------------------------------------------------------------
stopTrialProp  	= ones(nTrial, 1) .* pStopTrial;
...
distWindow = ones(nTrial, 1) * distWindow;
...
distAmp	= ones(nTrial, 1) * targAmp(1);  % for now target and distractor are equidistant from fixation

fixSize              = ones(nTrial, 1) * fixationWidth;
targSize                = ones(nTrial, 1) * targetWidth;
distSize            = ones(nTrial, 1) * distractorWidth;
checkerSize        = ones(nTrial, 1) * squareWidth * nCheckerColumn;
...
trialData = dataset(...
    ...
    {round(targDuration*1000),        'targDuration'},...
...
SessionData.task.effector = effector;
...
SessionData.stimuli.target1CheckerRGB   = [rightTargetCheckerColor];
SessionData.stimuli.target2CheckerRGB   = [leftTargetCheckerColor];
SessionData.stimuli.fixationRGB         = fixationColor;
SessionData.stimuli.targetRGB           = targetColor;
SessionData.stimuli.stopSignalTone      = stopHz;
...
% *******************************************************************
    function [window, timeout] = feedback(window, iTrialOutcome, iTrialType)
        ...
        elseif strcmp(iTrialOutcome, 'fixationAbort') || strcmp(iTrialOutcome, 'targetAbort')  || strcmp(iTrialOutcome, 'distractorAbort')
            DrawFormattedText(window, 'Please fixate on center', 'center', 'center', [190, 190, 190]);
            timeout = 1.5;
        ...
        elseif strcmp(iTrialType, 'go') && (strcmp(iTrialOutcome, 'targetHoldAbort') || strcmp(iTrialOutcome, 'distractorHoldAbort'))
            DrawFormattedText(window, 'Look at the target until it disappears ', 'center', 'center', incorrectTextColor);
            timeout = .2;
