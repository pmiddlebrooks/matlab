...
for trl = 1:size(EyeX_,1)
    if ~isnan(SRT(trl,1)) & SRT(trl,1) < 2000 & SRT(trl,1) > 50
        ...
        %for eye position traces, use 10 ms before saccade to 10 ms after saccade
         pos.X(trl,1:length(SRT(trl,1)-10:SRT_end(trl,1)+15)) = EyeX_(trl,SRT(trl,1) + Target_(1,1) - 10:SRT_end(trl,1) + Target_(1,1) + 15);
         pos.Y(trl,1:length(SRT(trl,1)-10:SRT_end(trl,1)+15)) = EyeY_(trl,SRT(trl,1) + Target_(1,1) - 10:SRT_end(trl,1) + Target_(1,1) + 15);
        ...
        %change in eye position will be 10 ms averages around saccade start and end
        loc_start_X = mean(EyeX_(trl,Target_(1,1) + SRT(trl,1)-10:Target_(1,1) + SRT(trl,1)));
        loc_start_Y = mean(EyeY_(trl,Target_(1,1) + SRT(trl,1)-10:Target_(1,1) + SRT(1,1)));
        
        %for end point, shift 5 ms into the future to make sure eyes are not still moving
        loc_end_X = mean(EyeX_(trl,SRT_end(trl,1) + Target_(1,1) + 5:SRT_end(trl,1) + Target_(1,1) + 15));
        loc_end_Y = mean(EyeY_(trl,SRT_end(trl,1) + Target_(1,1) + 5:SRT_end(trl,1) + Target_(1,1) + 15));
