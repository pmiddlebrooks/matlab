...
while t < p - 1
  ...
  % -----------------------------------------------------------------------
  u(any(blockInput(:,resp),2),t) = realmin;
  
  % Inhibition mechanism 2: lateral inhibition, if any
  % -----------------------------------------------------------------------
  ...
  dzdt        = At  * z(:,t)      * dt/tau + ...   % Endogenous connectivity
                C   * u(:,t)      * dt/tau + ...   % Inputs
                SI  * diag((u(:,t) ~= 0)) * randn(n,1)  * sqrt(dt/tau);  % Noise (intrinsic)
