function [x,fval,exitflag,output] = fminsearch_bbz(funfcn,x,options,varargin)
...
while func_evals < maxfun && itercount < maxiter
    ...
    if prnt == 3
        %fprintf(' %5.0f        %5.0f     %12.6g         %s\n', itercount, func_evals, fv(1), how)
        fprintf([' %5.0f        %5.0f     %12.6g         %s',repmat('%12.6g',1,numel(fv)-1),'%s ',repmat('%12.6g',1,numel(two2np1)),'\n'], itercount, func_evals, fv(1), how,abs(fv(1)-fv(two2np1)),' /// ',max(abs(v(:,two2np1)-v(:,onesn))))
    elseif prnt == 4
        disp(' ')
        disp(how)
        v
